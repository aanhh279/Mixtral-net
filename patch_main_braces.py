...
pattern = r'(\s*\) \{\s*Text\(\s*text = label,\s*color = if \(isSelected\) colorHex else Color\.White,\s*fontSize = 9\.sp,\s*fontWeight = FontWeight\.Bold,\s*fontFamily = FontFamily\.Monospace\s*\)\s*\}\s*\}\s*\})        \}            \}        \}'
